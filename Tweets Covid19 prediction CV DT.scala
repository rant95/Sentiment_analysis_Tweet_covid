...
// MAGIC 
// MAGIC ### DecisionTree ngram Cross validation
...
// MAGIC %md
// MAGIC ### Decision Tree Classifier CV
...
// COMMAND ----------
  ...
  // test data set - 30%
  val seed = 5043
  val Array(trainingData0, testData0) = featurizedData2.randomSplit(Array(0.9, 0.1), seed)
  val Array(trainingData1, testData1) = featurizedData2Test.randomSplit(Array(0.8, 0.2), seed)
...
val stages = Array(labelIndexer,dt)
...
val paramGrid = new ParamGridBuilder()
    ...
    //.addGrid(df.impurity, Array("entropy", "gini"))
    .build()
...
val bestModel = cv.bestModel.asInstanceOf[CVmodel].asInstanceOf[DecisionTreeClassificationModel]
display(bestModel)
