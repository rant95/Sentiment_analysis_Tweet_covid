...
display(withLemmesString.limit(7))
...
/*withTokens
    .select(explode(col("tokens")).as("token")).groupBy("token")
    .count().orderBy(col("count").desc).show(40)*/
...
val hashingTF = new HashingTF()
  .setInputCol("tokens").setOutputCol("rawFeatures").setNumFeatures(90000)

// COMMAND ----------

display(withTokens.limit(7))
...
// create the model
val nbcv = new NaiveBayes().setLabelCol("indexedLabel").setModelType("multinomial")
  .setFeaturesCol("rawFeatures").setSmoothing(0.1)
...
val CVmodel = nbcv.fit(trainingData)
...
//display(CVmodel, testData, plotType="ROC")
