...
val hashingTF = new HashingTF()
  .setInputCol("tokens").setOutputCol("rawFeatures").setNumFeatures(6000)
...
featurizedData.select("Feelings","filtered_lemmas").printSchema()
featurizedDataTest.select("Feelings","filtered_lemmas").printSchema()

// COMMAND ----------

val featurizedData2=featurizedData.select("Feelings","filtered_lemmas")
val featurizedData2Test=featurizedDataTest.select("Feelings","filtered_lemmas")
...
val pipeline = new Pipeline()
  .setStages(Array(tokenizer, hashingTF, naiveBayes))
...
val paramGrid = new ParamGridBuilder()
  .addGrid(hashingTF.numFeatures, Array(1000, 5000, 10000, 15000))
  .addGrid(naiveBayes.smoothing, Array(0.1))
  .build()
...
val predictionAndLabelsTest=cvPredictionDf.select("prediction","indexedLabel").rdd.map(f => (f.getAs[Double](0),f.getAs[Double](1)))
